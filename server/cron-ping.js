...
const installDep = async () => {
  ...
  await cp.exec(
    "npm i node-cron shelljs json-format random-words",
    function (err, stdout, stderr) {
      ...
      console.log(
        "Dependencies installed: [ node-cron, shelljs, json-format, random-words]"
      );
    }
  );
};
...
try {
  ...
  // ...
  const message = () => {
    // var items = ['progress', 'changes', 'color-fixes', 'bug-fixes', 'new functionality', 'updated packages', 'security-fix', 'ui-update', 'backend-fix'];
  ...
  const writingLogs = (newlogs) => {
    ...
    if (getLogs().state === "Empty") {
      ...
      if (lastKey === todayDate()) {
        let cutlogs = JSON.stringify(getLogs().data).slice(
          0,
          JSON.stringify(getLogs().data).length - 2
        );
        ...
        let cutlogs = JSON.stringify(getLogs().data).slice(
          0,
          JSON.stringify(getLogs().data).length - 1
        );
        logs =
          cutlogs +
          ',"' +
  ...
  let path = "../"; //your folder path (views is an example folder)
    ...
    fs.open(path, "r", (err, fd) => {
      if (err) {
        ...
        fs.stat(path, (err, data) => {
          console.log(
            "check if file/folder last modified date, was it after my last check "
          );
